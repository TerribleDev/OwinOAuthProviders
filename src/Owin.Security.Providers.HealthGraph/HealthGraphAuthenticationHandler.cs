...
namespace Owin.Security.Providers.HealthGraph
{
    public class HealthGraphAuthenticationHandler : AuthenticationHandler<HealthGraphAuthenticationOptions>
    {
        ...
        public HealthGraphAuthenticationHandler(HttpClient httpClient,ILogger logger)
        {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                var user = JObject.Parse(userText);

                // Get the RunKeeper Profile
                var profileRequest = new HttpRequestMessage(HttpMethod.Get, Options.Endpoints.ProfileInfoEndpoint + "?access_token=" + Uri.EscapeDataString(accessToken));
                var profileResponse = await _httpClient.SendAsync(profileRequest, Request.CallCancelled);
                profileResponse.EnsureSuccessStatusCode();
                var profileText = await profileResponse.Content.ReadAsStringAsync();
                var profile = JObject.Parse(profileText);
