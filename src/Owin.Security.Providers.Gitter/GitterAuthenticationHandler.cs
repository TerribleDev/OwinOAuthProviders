    ...
    public class GitterAuthenticationHandler : AuthenticationHandler<GitterAuthenticationOptions>
    {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                var userArray = JArray.Parse(text);
                var user = JObject.Parse(userArray[0].ToString());

                var context = new GitterAuthenticatedContext(Context, user, accessToken, tokenType)
                {
