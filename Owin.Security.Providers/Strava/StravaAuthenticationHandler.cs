    ...
    public class StravaAuthenticationHandler : AuthenticationHandler<StravaAuthenticationOptions>
    {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                JObject user = (JObject)response.athlete;
                ...
                if (!string.IsNullOrEmpty(context.DisplayName))
                {
                    context.Identity.AddClaim(new Claim("urn:strava:displayName", context.DisplayName, XmlSchemaString, Options.AuthenticationType));
                }
                if (!string.IsNullOrEmpty(context.Link))
                {
                    context.Identity.AddClaim(new Claim("urn:strava:url", context.Link, XmlSchemaString, Options.AuthenticationType));
                }
        ...
        public override async Task<bool> InvokeAsync()
        {
            if (Options.CallbackPath.HasValue &&
                !String.IsNullOrEmpty(Options.CallbackPath.Value) && 
                Options.CallbackPath == Request.Path)
            {
